...
class tx_nkwgok_updateCSV extends tx_scheduler_Task {
	...
	public function execute() {
		...
		if (!$success) {
			t3lib_div::sysLog('updateCSV Scheduler Task: Problem during conversion of CSV files. Stopping.' , tx_nkwgok_utility::extKey, t3lib_div::SYSLOG_SEVERITY_FATAL);
		}
		else {
			$loadxmlTask = t3lib_div::makeInstance('tx_nkwgok_loadxml');
			$success = $loadxmlTask->execute();
			if (!$success) {
				t3lib_div::sysLog('updateCSV Scheduler Task: could not import XML to TYPO3 database.' , tx_nkwgok_utility::extKey, t3lib_div::SYSLOG_SEVERITY_FATAL);
			}
		}
